...
public class AddPropertyServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int ownerId = 1; // Default fallback
        try {
        ...
        // PBO Compliance: Panggil addProperty() pada objek Owner
        ...
        // Handle Cover Photo — Upload ke Cloudinary
        ...
        if (coverPart != null && coverPart.getSize() > 0) {
            ...
            if (cloudinaryUrl == null) {
                ...
                response.getWriter().write("{\"success\": false, \"message\": \"Gagal upload cover photo ke Cloudinary. Coba lagi.\"}");
                return;
            }
            photoUrls.add(cloudinaryUrl);
        }

        // Handle Gallery Photos — Upload ke Cloudinary
        for (Part part : request.getParts()) {
            if ("gallery_photos".equals(part.getName()) && part.getSize() > 0) {
                ...
                if (cloudinaryUrl != null) {
                    ...
                    // Gallery photo gagal bukan fatal, lanjutkan saja
                }
            }
        }
        
        // Purged default mockup photo fallback. If empty, keep it empty.
        String photosJoined = photoUrls.isEmpty() ? "" : String.join(",", photoUrls);

        // Subclass logic: Capture parameters and map objects conforming to the Class Diagram
        Property prop = null;
        if ("Kost".equalsIgnoreCase(propertyType)) {
            ...
            Rumah r = new Rumah();
            try { r.setJumlahKamar(Integer.parseInt(request.getParameter("jumlahKamar"))); } catch(Exception e){}
            try { r.setLuasTanah(Double.parseDouble(request.getParameter("luasTanah"))); } catch(Exception e){}
            prop = r;
        } else if ("Kontrakan".equalsIgnoreCase(propertyType)) {
            Kontrakan kr = new Kontrakan();
            try { kr.setJumlahKamar(Integer.parseInt(request.getParameter("jumlahKamar"))); } catch(Exception e){}
            try { kr.setDurasiMinimum(Integer.parseInt(request.getParameter("durasiMinimum"))); } catch(Exception e){}
            prop = kr;
        } else if ("Apartement".equalsIgnoreCase(propertyType)) {
            Apartement a = new Apartement();
            try { a.setLantai(Integer.parseInt(request.getParameter("lantai"))); } catch(Exception e){}
